 ...
 * coloumns.
 * Make the cell of the tree editable.
 */
#include <gtk/gtk.h>
...
static void cell_edited( GtkCellRendererText *renderer, gchar *path,
    gchar *next_text, GtkTreeView *treeview );
...
static void setup_tree_view( GtkWidget *treeview ) {
  ...
  gtk_tree_view_append_column( GTK_TREE_VIEW( treeview ), column );
  /* set the third column in the tree to be editable */
  renderer = gtk_cell_renderer_text_new();
  g_object_set( renderer, "editable", TRUE, "editable_set", TRUE, NULL );
  g_signal_connect( G_OBJECT( renderer ), "edited", G_CALLBACK( cell_edited ),
      (gpointer) treeview );
  column = gtk_tree_view_column_new_with_attributes(
 ...
 */
static void cell_edited( GtkCellRendererText *renderer, gchar *path,
     gchar *new_text, GtkTreeView *treeview ) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  if ( g_ascii_strcasecmp( new_text, "" ) != 0 ) {
    model = gtk_tree_view_get_model( treeview );
    if ( gtk_tree_model_get_iter_from_string( model, &iter, path ) ) {
      gtk_tree_store_set( GTK_TREE_STORE( model ), &iter, PRODUCT,
	    new_text, -1 );
    }
  }
}
